...
router.get('/slack/connect', (req, res) => {
    const authURL = `https://slack.com/oauth/v2/authorize?client_id=${process.env.SLACK_CLIENT_ID}&scope=chat:write,channels:read,groups:read,im:read,mpim:read&redirect_uri=${process.env.SLACK_REDIRECT_URI}`;
